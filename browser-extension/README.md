...
1.2.0 优先读取当前站点的 `gld:sess` 和 `gld:sess.sig`，两项来自同一 Cookie 类型，并使用两项中较早的过期时间。旧的 `koa` Cookie 可能仍留在浏览器中，但在当前站点返回未登录。出现部分 `gld` Cookie 时会要求重新登录，不会混入旧签名。

从 1.1.0 升级：下载并解压新版，在扩展管理页重新加载新版目录，然后刷新签到后台；编辑旧账号，点击“一键读取浏览器 Cookie”，确认类型为 `gld` 后保存。仅更新服务端或把旧值换名称都不足以迁移登录态。
